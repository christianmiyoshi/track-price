(defproject projection-clojure "0.0.1-SNAPSHOT"
  ...
  :profiles {:dev {:aliases {"run-dev" ["trampoline" "run" "-m" "projection-clojure.server/run-dev"]}
                   :dependencies [[io.pedestal/pedestal.service-tools "0.5.10"]]}
             :uberjar {:aot [projection-clojure.server]}}
  :main ^{:skip-aot true} projection-clojure.server)
